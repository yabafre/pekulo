...
- **2026-05-06 — Finding 7: Vercel CI build OOM on `bun install` (verbatim log, occurred at 11:59:27 — concurrent with verdict push).** Vercel auto-built `feature/9-0-9-tamagui-spike` at commit `cb8a983` on its standard build tier (2 cores, 8 GB RAM). `bun install v1.3.6` resolved + extracted 118 packages in <2 s, then SIGKILL'd by the kernel after **13 min 8 s** with an OOM event during the post-resolve linking + post-install pass. Verbatim build output: `12:12:35.111 Error: Command "bun install" exited with SIGKILL` followed by Vercel's build-system OOM report. This turns finding 5/6 from a local-DX concern into a hard CI failure mode: the V1 deploy target (Vercel) cannot install the dependency tree on its standard build tier. The 8 GB ceiling is what Vercel's free + most paid plans ship; permanently upgrading the build-machine class would mean paying the Tamagui transpile chain's memory cost on every PR preview, every production deploy, every preview teardown. Not a story-defined pivot trigger (the pivot rules only cover AC-level fails) but it closes the last theoretical escape hatch — "maybe dev-tier cost is acceptable because CI does the heavy lifting" — by demonstrating CI is the WORSE-cost tier, not the better one. Documented as finding 7 in `docs/spikes/0-9-tamagui-decision.md`.

### Completion Notes

The spike resolved W2 with a clear **pivot** verdict. Seven independent findings (documented in `docs/spikes/0-9-tamagui-decision.md`) compounded to make the Tamagui v2-rc.41 + Next 16 Turbopack + bun monorepo + 16 GB Apple Silicon dev box + 8 GB Vercel build container combination unfit for V1:
...
2. **AC-2 SSOT-induced gaps** — when the test pairs were body-classified at verdict time, two pekulo-light pairs missed 4.5:1 (`accent.500 #059669` and `semantic.warning #D97706` on `#FFFFFF`). Post-pivot iso sweep (commit `2ee552d`) realigned tokens to TR-strict pure-black SSOT and refined to WCAG 1.4.11 (`indicator` ≥ 3:1) for non-text usage with one documented per-mode override (`accent.500 #00a852` on `#FAFAFA` at 2.99 = SSOT-iso); 16 / 16 pass under the corrected classification. The verdict trigger holds at verdict-time evidence; the sweep is a forward-looking iso for the V1.5 ramp.
...
7. **Vercel CI build OOM on `bun install`** — standard build tier (2 cores, 8 GB) SIGKILL'd at 13 min 8 s during post-resolve linking + post-install at commit `cb8a983`. CI cannot install the dep tree on Vercel's default build container, hardening findings 5/6 from local-DX into a hard deploy-target failure mode.
