...
- **RLS stays active on every user-scoped table** as defense in depth. The `rls-audit` CI probe still asserts every user-scoped table has 4 RLS policies ; build fails if any table is unprotected. Rationale: if `apps/web` ever bypasses `apps/api` and queries Supabase directly (Auth flows are the only sanctioned path today), or if an `apps/api` repository accidentally omits the `userId` guard, RLS catches the bug.
