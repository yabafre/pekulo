...
| Brandfetch (Brand Search + Logo Link)                                   | Merchant-logo resolution (server-side, cached), tier 1 of the transaction logo fallback (FR-65) | V1          | optional (`BRANDFETCH_API_KEY`; unconfigured → bank/category fallback) |
...
5. **Cache invalidation graph (`setTagRegistry`)** — in place; story 7-1 wired the dashboard read-aggregator (`dashboardKeys.overview()`) so accounts / holdings / realestate / transactions / compass list-tag mutations refresh the Cap view total wealth (FR-44). The dashboard is read-only (no `dashboardTags`). Codified as a Phase 3 hard rule. The dashboard `getOverview` (story 7-1) prices holdings LIVE via the holdings 4-tier chain behind the 60 s PricesCache (quotes deduped per `ticker|kind|currency` + bounded concurrency); NFR-1 (compass-progress < 300 ms p95) holds on cache-warm loads, cold-cache loads accept the NFR-2 price-chain latency as a documented deviation. The OVERVIEW half stays pure composition over accounts / holdings / realestate / compass ports, but its DTO gained `recentActivity` (server-resolved label / direction / amount / logo, story 7-2 D3). Story 7-2 also added the dashboard's first table — `dashboard_layout` (one JSONB `widgets` row per user, RLS-scoped `user_id = auth.uid()`) — behind `dashboard.getLayout` / `saveLayout`, powering the configurable widget grid (dnd-kit lazy-loaded behind `dynamic(ssr:false)`, out of the default bundle — NFR-3; FR-41 guaranteed on the default layout only; `dashboardLayoutTags.current()` invalidates only `dashboardLayoutKeys.layout()`). See ADR-0017.
...
- **Schema folder** at `apps/api/prisma/schema/` — one file per Elysia domain module:
  ...
  - `accounts.prisma`, `holdings.prisma`, `transactions.prisma`, `compass.prisma`, `realestate.prisma`, `llm.prisma`, `monthly.prisma`, `hypothesis.prisma`, `dashboard.prisma`, `bank_aggregator.prisma` (added 2026-05-25 per ADR-0015 — `BankConnection` model with `pgcrypto` column-level encryption on OAuth token columns), `logos.prisma` (added 2026-06-01 — story 6-10 / FR-65, the two logo reference caches; see "Reference caches" below)
...
- **Reference caches (non-user data, story 6-10 / FR-65):** `merchant_logo_cache` (PK `merchant_key`) and `provider_logo_cache` (PK `provider_id` = Bridge institution id) hold public brand / bank logo URLs for the 3-tier transaction-logo fallback (merchant → bank → category icon). They carry **no `user_id`, no FK to `auth.users`, and no RLS _policies_** — but RLS itself is **enabled** (migration `20260602150000`) with zero policies = deny-all to the anon PostgREST role (Supabase advisor `0013_rls_disabled_in_public`), so the only access path is `apps/api`'s service-role connection (bypasses RLS); their non-PII content is the secondary safeguard (both model names listed in `.oxlintrc.json` `unscopedModels` so `no-prisma-query-without-user-id` is satisfied ; both stay in `rls-migration-audit`'s `NON_USER_TABLES` because deny-all = 0 policies is intentional, not the ≥ 2 a user-data table needs). Semantics: a `logo_url = NULL` row is a NEGATIVE cache entry, refreshed on a per-tier TTL window (`logos.service.ts` — 14 d for misses, 90 d for hits) so a once-failed lookup recovers rather than sticking on the fallback forever. `accounts.provider_id` (migration `20260602100000`) stores the Bridge institution id at import so the bank-logo tier resolves on IBAN accounts whose dedup key (`iban:…`) carries none. **See ADR-0015.**
- **Pagination convention (D2): cursor-based** — every user-scoped list endpoint with potential row count > 100 (`transactions`, `llm_call_log`, `holding_lots`) uses `(createdAt desc, id desc)` keyset pagination with a `nextCursor` in the response. Offset pagination forbidden **except for two bounded, numbered-page UI views** that need random-access page N (which a cursor can't give): the pending-suggestion list (`listPendingByUser`) and the transactions **Récentes** view (`listByUser` `page` mode, story 6-9 ext). Both are small at Persona #1 scale (a few hundred rows), each returns a `totalCount`, and each is a conscious, documented deviation to revisit if a user nears the 50k cap; the cursor path remains the default for every other caller. A third, _distinct_ deviation: the `llm_call_log` **90-day activity-log read** (`listRecentOutcomesByUser`, story 6-5 / FR-36) keeps the `(createdAt desc, id desc)` keyset **ordering** but returns a bounded **200-row** window with **no `nextCursor`** — the web tier paginates that bounded window client-side (10/page), since at Persona #1 scale (≤ 200 LLM calls / 90 d) it cannot grow unbounded. Escalate to a server cursor/offset only if the 200 cap is raised past that scale. A **fourth** deviation, ratified in the aped-review of story 11-1: the GDPR portability export (`settings.export.ts#EXPORT_NODES`, `GET /v1/export`) reads each of the 21 user tables with an unpaginated `findMany` — `transactions`, `llm_call_log` and `holding_lots` included. A portability dump is by definition the complete set, so a `nextCursor` has no caller: the consumer is a file on disk, not a UI. Bounding is done one level up instead — the response is streamed one JSON fragment per table (AC-8), so peak memory tracks the LARGEST single table rather than the whole payload. The residual exposure is a single user whose largest table alone approaches the NFR-16 50k ceiling; revisit by making `ExportNode.read` a generator that emits `rows` page by page, which the chunked design already accommodates. Satisfies NFR-16.
...
- **Authorization (NFR-8 amended — defense in depth):**
  ...
  - **Public read exception — transaction-logo proxy (story 6-10 / FR-65):** `GET /v1/logos?ref=<opaque>` (Elysia-native on `apps/api`) and its same-origin forwarder (`apps/web/src/app/v1/logos/route.ts`) serve public reference bytes (brand / bank logos) carrying **no user data**, so `apps/web/src/proxy.ts` skips the per-request Supabase `getUser()` for the **EXACT** path `/v1/logos` only (a transactions page fires 20+ logo requests; gating each on an auth round-trip is slow and pointless). Anti-SSRF (AC-5): `ref` is an opaque base64url cache index decoded **server-side** to a cached upstream URL — never a caller-supplied address; the `apps/api` proxy additionally pins the resolved URL to `https` + a Brandfetch / Bridge host allowlist and relays `image/*` content-types only, and the web forwarder builds the api URL from `API_BASE_URL` + the ref (no client-controlled host).
  - **See ADR-0013.**
...
**Boundary rule — Components ↔ Actions** (hard; client components only): a (client) component never imports a Server Action directly. It calls a custom hook (local or global) that encapsulates the action plus its UX orchestration (optimistic state, success/error toast, query invalidation, navigation, retry policy). RSCs that call a server function directly are not bound by this (server-to-server). See ADR-0010.

> **Exception — Supabase Auth actions (story 8-1).** The `(auth)` client components (`auth-form.tsx`, `recover-form.tsx`) and the dashboard client `sign-out-button.tsx` import `(auth)/_actions/auth-actions.ts` **directly**, without an intervening hook. Those actions wrap Supabase Auth SDK calls (`signInWithPassword`/`signUp`/`signOut`/`resetPasswordForEmail`/`updateUser`) — the one documented direct-Supabase path (see Authentication & Security, "apps/web direct queries are limited to Supabase Auth API only") — **not** oRPC data reads, so there is no React Query cache for a hook to orchestrate and the hook layer would add ceremony with no invalidation value. This is an accepted, deliberate exception. (The layering exception is distinct from the direct-Supabase-query exception: the latter governs _which queries_ the web tier may issue, not the import boundary.)
>
> **Enforcement gap (tracked — follow-up, not 8-1).** `no-server-action-in-component` does **not** currently catch this class: its filename anchoring (`apps/web/src/…`) does not match the cwd-relative paths `oxlint src` feeds it from `apps/web`, so the rule is inert; and it has no `'use client'` gate, so naively fixing the anchoring false-positives on legitimate RSC server-to-server calls (e.g. `bank/callback/page.tsx`). Hardening it (cwd-relative anchoring + `'use client'` gate + a per-feature allowlist for the auth exception) is a dedicated lint-infra task, deliberately out of scope for this auth story.
...
│   │   │   │   │   ├── settings-actions.ts          updateTheme, updateLang (FR-51-52 ; FR-49 needs no action, FR-50 lives in (cap)/dashboard/_data/_actions — see FR → File Mapping)
...
| FR-49 | One-action JSON export                       | `apps/api/src/modules/settings/settings.export.ts#streamUserDataExport` behind the Elysia-native `settings.export-routes.ts` (`GET /v1/export`) — a stream cannot travel through the oRPC envelope, so this is NOT an oRPC procedure (story 11-1)                                                                                          | `app/v1/export/route.ts` (same-origin authenticated proxy) + `(cap)/dashboard/_data/_components/{data-section,export-data-row}.tsx`. **No hook, no server action**: ADR-0010's triad governs oRPC calls and the browser makes none — the anchor's `download` plus the proxy's `Content-Disposition` do the work (story 11-1)                                                                                |
| FR-50 | Cascade account deletion < 60 s              | `settings.service.ts#deleteAccount` — Bridge erasure first (fail-closed, `bank-aggregator.service.ts#eraseUserAtProvider`), then `settings.deletion.ts#deleteUserData` (explicit `DELETION_NODES` fan-out over all 21 user tables in one transaction), then `platform/auth#createSupabaseAuthAdmin` (`auth.admin.deleteUser`) (story 11-2) | `(cap)/dashboard/_data/_components/{delete-account-row,delete-account-confirm}.tsx` → `_hooks/use-delete-account.ts#useDeleteUserAccount` → `_actions/data-actions.ts#deleteUserAccount` (ADR-0010 triad; typed failures return as an `{ ok: false, code }` envelope — `BANK_PROVIDER_UNAVAILABLE`, `ACCOUNT_PARTIALLY_ERASED`, `FORBIDDEN` — and the success path clears the session then `redirect("/")`) |
...
| FR-59 | Compare projection vs cap-required, gap in €/month                | `dashboard.service.ts#getHypothesisGap(userId, currentWealthEur)` → full `HypothesisGap` (`gapEurPerMonth`, `reachesCap`, `projectedFinalEur`, `requiredFinalEur`, `deltaAtCapEur`, `horizonYears`, linear `requiredPoints`), `null` when no compass; pure math in `derive/hypothesis-gap.ts` (`computeMonthlyGap` + `computeRequiredCurve`) | `dashboard/_hypothesis/_components/hypothesis-card.tsx` (`PekuloProjectionChart` + `PekuloHypothesisVerdict` w/ `gapEurPerMonth` line) + read-only `hypothesis-settings.tsx` (Paramètres) |
