...
- **L8 — Prisma 7 evaluates `env()` at config-load.** `prisma format --check` and `prisma validate` look static but Prisma 7's `defineConfig` resolves `env<Env>("DATABASE_URL")` at module-load time, BEFORE deciding which subcommand runs. The `prisma-check` job in `pr.yml` ships a stub `DATABASE_URL: "postgresql://stub:stub@localhost:5432/stub"` env var so the loader resolves ; the stub is never connected to. Any future CI/Docker context invoking no-connection prisma commands must repeat the stub.
