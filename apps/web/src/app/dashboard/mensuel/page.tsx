...
export default async function MensuelPage() {
  const [{ hypotheses }, actuals] = await Promise.all([readHypotheses(), readMonthlyEntries()]);

  const actualByKey = new Map(actuals.map((entry) => [`${entry.year}-${entry.monthNum}`, entry]));
    ...
    <div className="mx-auto max-w-6xl px-4 sm:px-6 py-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Saisis tes vrais chiffres mois par mois. Les lignes sans saisie utilisent la projection
          issue de tes paramètres.
        </p>
      </div>
