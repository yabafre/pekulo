...
9. Bank-account connectivity — Bridge (`bridgeapi.io`) for supported French and EU banks (Société Générale, Revolut, …), via the agent-of AISP route documented in ADR-0015. Trade Republic and other unsupported sources keep the CSV-import path (FR-29). Token storage encrypted at rest ; SCA refresh every 90 days surfaces a reconnect CTA. Resolves DR-9.
...
3. Wallet / exchange connectivity for crypto — exchange APIs (Binance, Kraken, …) and optional xpub for cold wallets. (Bank connectivity promoted to V1 on 2026-05-25 via ADR-0015 ; Powens parked as a candidate for the (c) freemium tier when its Wealth product becomes load-bearing for the Finary-scope vision.)
...
- ~~Bank-account connectivity at V1 / V1.5 entry~~ — **promoted to V1 on 2026-05-25** via an explicit `aped-course` correction (the mechanism this section anticipates) and ADR-0015 (Bridge as agent-of AISP). Original exclusion preserved here for audit ; see Group E FR-60 to FR-63 for the introduced functional surface and DR-9 for the resolved regulatory posture.
...
### J10 — Connect a bank ("Bridge takes over")

Alex opens the settings → connections page, taps "Connecter une banque", and is redirected to the Bridge-hosted Connect widget. He authenticates to Société Générale (or Revolut) via the bank's SCA flow, returns to Pekulo with the connection persisted, and the Récentes section on the transactions screen starts populating automatically from the cron-refresh job. After 90 days, the next refresh returns `SCA_REQUIRED` ; the connections page surfaces a "Reconnecter" CTA, Alex re-runs the SCA flow, and refresh resumes. Trade Republic remains via story 5-2 CSV import (TR is not currently a reliable Bridge connector and only represents broker / PEA / CTO flow in the persona stack).
...
| Pekulo connects to user banking via PSD2 / open banking.                                     | Yes at V1 via Bridge as agent-of AISP per ADR-0015 (2026-05-25). Bridge is ACPR-approved AISP + PISP ; Pekulo operates under the tied-agent business-customer relationship documented in the Bridge B2B contract. Own AISP licence remains out of scope. |
...
| PSD2 / DSP2 (open banking)                                             | EU                       | Yes at V1                                                                    | Triggered at V1 via Bridge as agent-of AISP per ADR-0015 (2026-05-25).                                  | Bridge B2B contract must document the agent-of / tied-agent relationship. Production traffic gated on signed Bridge production contract. Own AISP licence remains out of scope. Pivot conditions documented in ADR-0015. |
...
| Bank credentials (Bridge) | Bridge OAuth access + refresh tokens stored encrypted at rest via Supabase `pgcrypto` column-level encryption. Never logged. Never returned by oRPC handlers (DTO mapping strips secret columns). SCA-required state surfaced in the connections page ; no silent retry on expired SCA. | Token rotation on SCA refresh ; audit log of token operations (create / refresh / revoke). Webhook signature verification mandatory ; production endpoints behind HMAC. | Per ADR-0015. Bridge B2B contract documents the agent-of regulatory route resolving DR-9.                                   |
...
- FR-60: User can connect a bank account via the Bridge-hosted Connect widget (OAuth + SCA) from the settings connections page, with the resulting `BankConnection` row persisted under the user's id and encrypted token columns.
...
- NFR-31: The system shall store all Bridge OAuth access and refresh tokens encrypted at rest via Supabase `pgcrypto` column-level encryption, with 0 token values returned by any oRPC handler and 0 token values written to any log destination, verified by a pre-(b) automated check that fails the build on a regression.
...
- NFR-32: The system shall refresh Bridge bank-connection consents within 90 days of each SCA, surfacing a `SCA_REQUIRED` state in the user's connections page when refresh fails, with 0 silent failures and 0 auto-retry attempts on expired SCA.
- NFR-33: The system shall verify every incoming Bridge webhook payload against the configured HMAC signature, rejecting unsigned or invalid-signature requests with a 401 within 100 ms.
