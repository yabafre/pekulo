...
**Post-fix verification pass (2026-08-25, second round).**

Aria and the Code auditor were re-dispatched against the fix commits and both reproduced the gates themselves rather than trusting the numbers. **All findings confirmed RESOLVED, no regression in the 52 pre-existing snapshots.** Aria verified the focus ring *empirically* — a literal two-rule probe in the harness computes `outline-width: 2px` after `.focus()`, confirming specificity `(0,2,0)` beats the `(0,1,0)` reset — and noted that the real control computes `initial` only because `--color` is undefined in the test harness, the same construct `PekuloButton` has always shipped.

Both passes found real defects **in the review's own fixes**, which were then fixed in `4649aec`:

- **The `PekuloDatePicker` radius took the wrong reference (Aria).** `ux-preview:896-921` is explicitly the pill "for inline section actions (Ajouter / Filtrer / Voir tout)". The DatePicker is never used that way: all **7** consumers render it inside a `<PekuloField>` with a `<PekuloFieldLabel htmlFor>`, stacked between a `PekuloNativeSelect` and a `PekuloInput` — both at `pekuloRadius.lg` (12px). `borderRadius: 9999` made it a pill in a column of fields. Corrected to `pekuloRadius.lg`; the border removal (the actual §1 violation) stands. ux-preview contains no form controls at all, so it cannot arbitrate a date field — the citation did not carry.
- **`NAV_LABELS` was redundant and actively harmful (Aria).** Probed: `<DayPicker locale={fr} />` with no `labels` already returns `"Aller au mois précédent"` / `"suivant"`, byte-identical. Worse, hardcoding them survives a consumer's `locale={enUS}`, leaving French buttons on an English calendar. Removed; the captures are unchanged.
- **The new gate reproduced its own failure class in narrower form (Code auditor).** Demonstrated with synthetic probes: `barrelModules()` missed `export { X } from "./y"`, sub-paths and multi-line exports; `moduleComponents()` missed `export const Foo = () => …`; and `moduleSpecMentions` used `covered.includes(name)`, so `"PekuloToastViewport"` would have marked `PekuloToast` covered on its sibling's import line. None was exploited — the barrel uses one shape and `PekuloToast` has its own specs — but all three were regex/substring assumptions replacing hard-coded folder names. Widened to accept every shape, and the coverage check is now word-boundary. Re-probed: a component added as an **arrow function** in `src/toast/` now fails the gate, which it did not before.

Two items were left open by deliberate decision, both recorded above: the `<style>` block is emitted per instance (pre-existing pattern, now on three more primitives), and the floor assertion catches enumeration collapse but not a partial regression — self-documented as such in the file.
