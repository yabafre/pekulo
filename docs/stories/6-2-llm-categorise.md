...
- **Decisions locked at step 04 (this story):**
  ...
  - **Q2 — categorisation is async fire-and-forget, `createTransaction` path only** (NFR-1: off the hot path). Bulk import (`importCsv` / `importFromProvider`) keeps transfer-rule categorisation only; LLM batch suggestions (DR-3) are a later story. **Superseded by 6-7 (2026-06-02):** bulk import (csv/bridge) now AUTO-APPLIES the LLM category via the source-aware backfill; the create-path-only / suggest-only statement holds for source='manual' rows only.
