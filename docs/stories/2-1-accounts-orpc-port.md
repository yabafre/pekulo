...
- Migration (T1) NOT auto-applied: needs `bun --filter='@pekulo/api' run prisma:migrate:deploy` against the live Supabase pooler. The SQL is hand-written per the story-1-1 deviation (Supabase pooler hangs on `prisma migrate dev`). RLS policy count (`accounts: 4`) is preserved — the column-type flip does not touch `auth.uid() = user_id` policies.
