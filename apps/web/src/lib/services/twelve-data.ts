...
export class TwelveDataError extends Error {
  code: "missing-key" | "invalid-symbol" | "rate-limited" | "network" | "format" | "no-price";
...
export async function fetchTwelveDataQuote(symbol: string): Promise<TwelveDataQuote> {
  ...
  if (!apiKey) {
    throw new TwelveDataError("missing-key", "TWELVE_DATA_API_KEY non défini dans .env.local");
  }
