...
5. **Token storage** — Bridge OAuth tokens encrypted at rest in Supabase via `pgcrypto` column-level encryption ; never logged ; never returned by oRPC handlers (DTO mapping strips the secret columns).
   - **Amendment (2026-05-27, story 5-6, draft):** the literal storage mechanism switches from `pgcrypto` column-level encryption to **Supabase Vault** (`supabase_vault` extension). `bank_connections` carries `access_token_secret_id uuid REFERENCES vault.secrets(id) ON DELETE SET NULL` + `refresh_token_secret_id uuid REFERENCES vault.secrets(id) ON DELETE SET NULL` instead of `*_cipher bytea` columns.
   - **Amendment (2026-05-27, story 5-6, post-impl pivot):** the live Bridge integration revealed that Bridge v3 has moved to a **stateful-widget model** — no per-item OAuth tokens are issued to the integrator. Bridge mints a short-lived **user-level Bearer** (via `POST /v3/aggregation/authorization/token { user_uuid }`) that the API mints on demand for any subsequent user-scoped call ; the long-lived secret never leaves Bridge's infrastructure. Consequence: Pekulo carries `bank_connections.access_token_secret_id` and `refresh_token_secret_id` columns from the §5-draft amendment as **vestigial NULL columns** — Vault is enabled in the migration but **never written**. The `BridgeProvider` impl re-mints the Bearer per call via `mintUserAccessToken(userUuid)`. The §5 invariants still hold trivially (nothing to leak — there are no tokens persisted on Pekulo's side). A **V1.5 follow-up migration** will `ALTER TABLE bank_connections DROP COLUMN access_token_secret_id, DROP COLUMN refresh_token_secret_id` and roll back the `supabase_vault` extension if no other Pekulo surface adopts it. See `docs/stories/5-6-bridge-connector.md` § "Deviations" for the full pivot trail and the corresponding lesson at the top of `docs/lessons.md` (2026-05-27 correction — Bridge v3 3-tier auth).
...
- **Story 6-10 (FR-65) — bank-logo tier + shared provider (added 2026-06-02, aped-review doc-sync).** `BankProvider` gained `getProviderLogo(providerId) → { logoUrl }`, backed by Bridge `GET /v3/providers/:id → images.logo` — **app-level auth** (Client-Id / Secret headers, NO user Bearer; Providers is the public bank directory). The endpoint shape was validated against the canonical Postman collection (`docs/ressources/Bridge API.postman_collection.json`, `Providers / Get a single provider`), the live smoke being waived (sandbox creds are Dokploy-only). To break the `logos → provider → bank-aggregator` dependency cycle, the Bridge `provider` was extracted from `createBankAggregatorModule` to `runtime-dependencies.ts` and injected, so the logos module and the bank-aggregator share one instance (`createBankAggregatorModule` keeps an optional `provider?` defaulting to the internal one, so its unit tests stay green). A new `accounts.provider_id` column (migration `20260602100000`) stores the Bridge institution id at import time, so the bank-logo tier resolves on IBAN accounts whose stable key (`iban:…`) carries no provider id. See `docs/stories/6-10-merchant-logos.md` for the full trail. NFR-12 (zero PII to LLM) remains unaffected — the logo never enters the prompt envelope (AC-4 regression test). **RLS amendment (2026-06-02):** `merchant_logo_cache` / `provider_logo_cache` now have RLS **enabled with no policies** (migration `20260602150000`, deny-all to the anon PostgREST role per Supabase advisor `0013_rls_disabled_in_public`); the service-role connection bypasses RLS so resolution is unaffected. This refines the original "no RLS, no-PII" posture (architecture.md L131) — "no PII" justifies omitting policies, not omitting RLS itself.
