...
### Amendment (2026-06-05) — direct drag-resize

Post-review, the scope was extended (user override, kept in 7-2) to **direct widget resizing on the grid** — width AND height. The `DashboardWidget` record gains optional `colSpan` (1–12) + `rowSpan` (1–4), defaulting to the registry span; the read grid renders inline `grid-column`/`grid-row` spans on a generic `.bentoCell` fill class; the edit mode IS the bento grid (drag handle = reorder via dnd-kit `rectSortingStrategy`, a visibility switch, and a bottom-right resize handle snapping spans to the grid units). Consequence: the layout is now a free-form bento, not a fixed-span one — overlaps/reflow are resolved by CSS grid auto-placement; resize is pointer-only at V1 (keyboard resize deferred). Two runtime 500s surfaced only in live testing because the dashboard unit/integration tests stub the DB — the `dashboard_layout` migration had not been `migrate deploy`'d, and `DashboardLayout` was missing from `id-prefixes.config.ts` (the prefixed-ids extension threw on the `saveLayout` upsert create). Both fixed; the prefixed-ids gap is recorded as a lesson (2026-06-05).
