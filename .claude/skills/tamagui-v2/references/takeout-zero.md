...
const [user] = useQuery(userById, { userId }, { enabled: Boolean(userId) });
```
...
export const postsWithBlocks = (props: { blockedUserIds: string[]; pageSize: number }) => {
  return zql.post
    .where(permission)
    .where((eb) => {
...
export const complexSearch = (props: { searchText: string; blockedUserIds: string[] }) => {
  return zql.post.where(permission).where((eb) => {
...
export const allPosts = syncedQuery(
  "allPosts",
  v.parser(v.tuple([v.object({ limit: v.optional(v.number()) })])),
...
const permission = serverWhere("post", (q, auth) => {
  return q.or(q.cmp("published", "=", true), q.cmp("userId", "=", auth?.id || ""));
});
...
function PostActions({ post }) {
  ...
  return <Button onPress={() => editPost(post)}>Edit</Button>;
}
```
...
export const isUsersOwn = serverWhere<"userPublic">((q, auth) => q.cmp("id", "=", auth?.id || ""));
...
export const userRelationships = relationships(tables.userPublic, ({ one, many }) => ({
  posts: many({
    sourceField: ["id"],
...
export const allRelationships = [userRelationships, postRelationships];
```
...
export const postWithUser = (props: { postId: string }) => {
  return zql.post.where("id", props.postId).one().related("user");
};
...
export const postWithComments = (props: { postId: string }) => {
  return zql.post
    ...
    .one()
    .related("user")
    .related("comments", (q) => q.orderBy("createdAt", "desc").limit(50).related("user"));
};
```
...
function PostDetail({ postId }) {
  ...
  return (
    <YStack>
      ...
      <Button onPress={handleLike}>Like ({post.likeCount})</Button>
    </YStack>
  );
}
```
...
<ProvideZero userID={user.id} auth={jwtToken} authData={{ id: user.id, role: user.role }}>
  <App />
</ProvideZero>;
    ...
    if (server && authData) {
      ...
      server.asyncTasks.push(async () => {
        ...
        await server.actions
          .analyticsActions()
          .logEvent(authData.id, "post_created", { postId: post.id });
...
```ts
type QueryStatus = { type: "loading" } | { type: "error"; error: string } | { type: "complete" };
```
