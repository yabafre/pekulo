...
| 7 — Vercel CI build OOM              | Hard CI failure mode                        | **Resolved (mitigation b adapted)** — root cause confirmed: `bun install`'s parallel linker peaks > 8 GB on the full monorepo dep tree (Tamagui rc.41 + Prisma postinstall + OpenTelemetry + 7 workspaces). `--ignore-scripts` and `--filter` were insufficient (linking itself stays heavy). Final fix mirrors `tamagui/starter-free`: yarn 4.5 for Vercel install only (sequential linker, ~3 GB peak), bun runtime everywhere else (`bunVersion: "1.x"` + `bun --bun next build` per Bun-on-Vercel doc). `bun.lock` remains local SoT; Vercel resolves fresh each build (acceptable preview-build drift). 5 MB yarn 4.5.0 binary committed at `.yarn/releases/`. Last green build: `pekulo-671ttl95m` (~2 min, all 9 CI jobs green). Path (c) Dokploy migration kept on the shelf for V1 ramp public. |
...
**Post-review revision (current):** Run `aped-story` for `0-10-pekulo-ui-migration` to scaffold the migration sub-stories. The retrofit applied in this commit (slim `transpilePackages`, `dev: next dev`, `@tamagui/core/reset.css` import) means 0-10 inherits a Tamagui spike route that mirrors the official starter's wire-up — no rework required at 0-10 kick-off. Finding 7 (Vercel CI OOM) is now resolved on this branch via mitigation path (b) adapted — yarn 4.5 install + bun runtime everywhere else; Story 0-10 just needs to re-measure dev-tier cost on a fresh `bun run dev:web` session against the retrofit baseline (findings 5/6). ADR-0007 stays `accepted` — the original migrate-now decision holds.
