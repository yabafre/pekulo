...
**Boundary rule — Components ↔ Actions** (hard, lint-enforced via `no-server-action-in-component` — covers BOTH the global `@/lib/actions/` root and per-feature `_actions/` dirs): a component never imports a Server Action directly. It calls a custom hook (local or global) that encapsulates the action plus its UX orchestration (optimistic state, success/error toast, query invalidation, navigation, retry policy). See ADR-0010.

> **Exception — Supabase Auth actions (story 8-1).** The `(auth)` components (`auth-form.tsx`, `recover-form.tsx`) and the dashboard `sign-out-button.tsx` import `(auth)/_actions/auth-actions.ts` **directly**, without an intervening hook. Those actions wrap Supabase Auth SDK calls (`signInWithPassword`/`signUp`/`signOut`/`resetPasswordForEmail`/`updateUser`) — the one documented direct-Supabase path (see Authentication & Security, "apps/web direct queries are limited to Supabase Auth API only") — **not** oRPC data reads, so there is no React Query cache for a hook to orchestrate and the hook layer would add ceremony with no invalidation value. The lint rule allowlists `(auth)/_actions/` (`.oxlintrc.json` `allow`); this is the **only** exception — every other feature MUST route through a hook. (Note: the layering exception is distinct from the direct-Supabase-query exception; the latter governs _which queries_ the web tier may issue, not the import boundary.)
