...
import {
  COMPASS, MILESTONES, WEALTH, USER, TODAY, TRANSACTIONS, ACCOUNTS,
  HOLDINGS, PROPERTIES, MONTHLY, LLM_LOG, HYPOTHESIS,
} from './data/mock'
import { formatEUR, formatPct, formatCompactEUR, formatDate, formatMonthYear, signed } from './lib/format'
import { cn } from './lib/cn'
import {
  Compass, ChevronDown, Plus, Check,
  Receipt, LineChart, Wallet, Building2, Settings,
  ArrowUpRight, ArrowDownRight, Sparkles, Search, Globe,
  Moon, Sun, Monitor, Download, Trash2, LogOut, Bot,
} from 'lucide-react'
      ...
      <div className="flex flex-col min-h-dvh lg:pl-24 lg:pr-4">
        ...
        <header className="lg:hidden flex items-center justify-between px-5 pt-6 pb-2">
          {activeNav === 'cap'
            ...
            : <h1 className="text-body-lg lg:text-h1 font-semibold text-fg leading-none">{SCREEN_TITLE[activeNav]}</h1>}
...
function NavRail({ activeKey, setActiveKey }: { activeKey: NavKey; setActiveKey: (k: NavKey) => void }) {
  return (
    <aside
      aria-label="Navigation principale"
    ...
    >
      ...
      <button
        ...
        className={cn(
          ...
          activeKey === 'settings' ? 'bg-elevated text-fg' : 'text-fg-tertiary hover:text-fg hover:bg-muted'
        )}
      >
  ...
  return (
    <div className="flex flex-col gap-10 lg:max-w-3xl lg:mx-auto">
      <section>
        ...
        <p className="mt-2 text-h1 lg:text-hero tabular-nums text-fg">{formatEUR(WEALTH.totalEur)}</p>
        <p className="mt-2 text-body-sm text-fg-tertiary tabular-nums">
          {formatCompactEUR(WEALTH.cashEur)} liquide · {formatCompactEUR(WEALTH.holdingsEur)} placé · {formatCompactEUR(WEALTH.realEstateEur)} immobilier
        </p>
      </section>
      <AccountsSection />
      <CompositionSection />
...
function HeroBlock({ variant }: { variant: 'mobile' | 'card' }) {
  ...
  return (
    <section aria-label="Patrimoine total">
      <p className="text-caption text-fg-tertiary">{variant === 'mobile' ? 'Aujourd\'hui' : 'Patrimoine total'}</p>
        ...
        <>
          <dl className="mt-8 grid grid-cols-2 gap-x-6 gap-y-1">
            <div>
              ...
              <dd className="text-body lg:text-h2 tabular-nums text-fg mt-1 font-semibold">{formatEUR(COMPASS.targetCapital)}</dd>
              <dd className="text-caption text-fg-tertiary">en {COMPASS.targetYear}</dd>
            </div>
            <div>
              <dt className="text-caption text-fg-tertiary">Plan / an</dt>
              <dd className="text-body lg:text-h2 tabular-nums text-fg mt-1 font-semibold">{formatCompactEUR(WEALTH.required12mEur)}</dd>
...
function MiniKpis() {
  ...
  return (
    <section className="grid grid-cols-3 gap-3">
      <KpiTile label="Cap" valueTop={`${(pct * 100).toFixed(1)} %`} valueBottom={`${formatCompactEUR(remaining)} restants`} progress={pct} />
      <KpiTile label="Horizon" valueTop={`${COMPASS.targetYear}`} valueBottom={`${yearsLeft} ans`} />
      <KpiTile label="Plan / an" valueTop={formatCompactEUR(WEALTH.required12mEur)} valueBottom="linéaire" />
    </section>
  )
}
  ...
  return (
    <section aria-labelledby="traj-h">
      <header className="flex items-center justify-between mb-5">
        <h2 id="traj-h" className="text-h3 lg:text-h2 text-fg">Trajectoire</h2>
  ...
  return (
    <section aria-labelledby="ms-h">
      <header className="flex items-center justify-between mb-3">
        <h2 id="ms-h" className="text-h3 lg:text-h2 text-fg">Paliers</h2>
...
function CompositionSection() {
  ...
  return (
    <section aria-labelledby="comp-h">
      <h2 id="comp-h" className="text-h3 lg:text-h2 text-fg mb-3">Composition</h2>
...
function RecentActivitySection() {
  ...
  return (
    <section aria-labelledby="act-h">
      <header className="flex items-center justify-between mb-3">
        <h2 id="act-h" className="text-h3 lg:text-h2 text-fg">Activité récente</h2>
...
function AccountsSection() {
  ...
  return (
    <section aria-labelledby="acc-h">
      <header className="flex items-center justify-between mb-3">
        <h2 id="acc-h" className="text-h3 lg:text-h2 text-fg">Comptes</h2>
        <HeaderAction icon={Plus} label="Ajouter" />
      </header>
      <ul className="flex flex-col">
        {ACCOUNTS.map((a) => (
...
function HypothesisSection() {
  return (
    <section aria-labelledby="hyp-h">
      <header className="flex items-center justify-between mb-3">
        <h2 id="hyp-h" className="text-h3 lg:text-h2 text-fg">Hypothèse</h2>
        <span className="text-caption text-fg-tertiary tabular-nums shrink-0">
          {formatEUR(HYPOTHESIS.monthlyContributionEur)} / mois · {HYPOTHESIS.assumedAnnualRatePct.toFixed(1)} % / an
        </span>
      </header>
      <ProjectionChart />
      <HypothesisVerdict />
    ...
    <div className="mt-4 pt-4 border-t border-border flex items-center justify-between gap-4 flex-wrap">
      ...
      <div className="text-right tabular-nums shrink-0">
        <p className={cn('text-body-sm font-medium', exceeds ? 'text-gain' : 'text-loss')}>
          {exceeds ? '+' : '−'}{formatEUR(Math.abs(gap))}
        </p>
...
function ProjectionChart() {
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-auto" role="img" aria-label="Projection patrimoine versus cap requis">
        <line x1={padX} y1={h - padY} x2={w - padX} y2={h - padY} stroke="var(--chart-grid)" strokeWidth="1" />
        <path d={toPath('required')} fill="none" stroke="var(--chart-plan)" strokeWidth="1.5" strokeDasharray="4 4" />
        <path d={toPath('projected')} fill="none" stroke="var(--chart-actual)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        <circle cx={lastX} cy={lastYProj} r="4" fill="var(--chart-actual)" />
        <circle cx={lastX} cy={lastYReq} r="3" fill="none" stroke="var(--chart-plan)" strokeWidth="1.5" />
      </svg>
      <div className="mt-2 flex items-center justify-between flex-wrap gap-2 text-caption text-fg-tertiary">
        <span className="flex items-center gap-2">
          ...
          <span className="inline-block w-3 h-px border-t border-dashed border-fg-tertiary ml-3" /> Cap requis
        </span>
    ...
    <Section className={className} ariaLabel="Progression du cap">
      <div className="flex h-full flex-col items-center justify-center gap-6 text-center">
        ...
        <div>
          ...
          <p className="mt-1 text-body lg:text-h2 tabular-nums text-fg font-semibold">{formatEUR(remaining)}</p>
 ...
 */
function Section({
  className, ariaLabel, title, action, children,
}: {
  className?: string
  ariaLabel?: string
  title?: string
  ...
  return (
    <section
      ...
      className={cn('rounded-xl bg-card p-5 lg:p-6', className)}
    >
      {(title || action) && (
 ...
 */
function CountUpEUR({ value, className, precise }: { value: number; className?: string; precise?: boolean }) {
  const reduce = useReducedMotion()
...
function EmptyState({
  icon: Icon, title, message, ctaLabel, onCta,
}: {
  icon: typeof Compass
  title: string
  message: string
...
function HeaderAction({
  icon: Icon, iconRight, label, onClick,
}: {
  icon?: typeof Compass
...
function KpiTile({
  label, valueTop, valueBottom, progress,
}: { label: string; valueTop: string; valueBottom: string; progress?: number }) {
  return (
...
function MilestoneRow({ milestone }: { milestone: typeof MILESTONES[number] }) {
  ...
  const deltaCls =
    milestone.status === 'ahead'  ? 'text-gain'
  : milestone.status === 'behind' ? 'text-loss'
  : 'text-neutral'
  ...
  return (
    <div className="flex items-center gap-4 py-3">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-body-sm text-fg font-medium truncate">{milestone.label.replace(/^[^—]*— /, '')}</p>
      ...
      <div className="text-right shrink-0">
        <p className={cn('text-body-sm tabular-nums font-medium', deltaCls)}>
          {milestone.deltaEur >= 0 ? '+' : ''}{formatEUR(milestone.deltaEur)}
        </p>
        <p className="text-caption text-fg-tertiary">
          {milestone.status === 'ahead' ? 'en avance' : milestone.status === 'behind' ? 'en retard' : 'sur la trajectoire'}
        </p>
      </div>
    </div>
  )
...
function CompositionRow({
  label, amount, pct, sub,
}: { label: string; amount: number; pct: number; sub?: string }) {
  return (
    <div className="flex items-center gap-4 py-3">
...
function Donut({
  pct, size, stroke, centered,
}: { pct: number; size: number; stroke: number; centered?: boolean }) {
  const reduce = useReducedMotion()
  ...
  return (
    <div className={cn('relative inline-block', centered && 'mx-auto')} style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} role="img" aria-label={`${formatPct(pct)} atteint`}>
        <circle cx={cx} cy={cx} r={r} fill="none" stroke="var(--donut-track)" strokeWidth={stroke} />
        <motion.circle
          cx={cx}
          cy={cx}
          r={r}
...
function TrajectoryChart({ inCard, tall }: { inCard: boolean; tall?: boolean }) {
  ...
  const Inner = (
    <>
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-auto" role="img" aria-label="Trajectoire patrimoine 12 mois">
        <line x1={padX} y1={h / 2} x2={w - padX} y2={h / 2} stroke="var(--chart-grid)" strokeWidth="1" />
        <path d={toPath('plan')} fill="none" stroke="var(--chart-plan)" strokeWidth="1.5" strokeDasharray="4 4" />
        <path d={toPath('actual')} fill="none" stroke="var(--chart-actual)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
...
function compositionSlices() {
  return [
    { label: 'Liquide',    sub: 'Livret + courant',     amount: WEALTH.cashEur,       pct: WEALTH.cashEur / WEALTH.totalEur },
    { label: 'Placements', sub: 'PEA + CTO + AV',       amount: WEALTH.holdingsEur,   pct: WEALTH.holdingsEur / WEALTH.totalEur },
    { label: 'Immobilier', sub: 'Lyon 7e — net equity', amount: WEALTH.realEstateEur, pct: WEALTH.realEstateEur / WEALTH.totalEur },
  ]
}
...
function TransactionsScreen() {
  ...
  const totalInflow = TRANSACTIONS.filter((t) => t.type === 'inflow').reduce((s, t) => s + t.amountEur, 0)
  const totalOutflow = TRANSACTIONS.filter((t) => t.type === 'outflow').reduce((s, t) => s + Math.abs(t.amountEur), 0)
    ...
    <div className="flex flex-col gap-6 lg:gap-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Section ariaLabel="Net du mois">
          ...
          <p className={cn('mt-2 text-h2 tabular-nums', totalInflow - totalOutflow >= 0 ? 'text-gain' : 'text-loss')}>
        ...
        <Section ariaLabel="Entrées du mois" className="hidden lg:block">
          ...
          <p className="mt-2 text-body lg:text-h2 tabular-nums text-fg font-semibold">{formatEUR(totalInflow)}</p>
        </Section>
        <Section ariaLabel="Sorties du mois" className="hidden lg:block">
          <p className="text-caption text-fg-tertiary">Sorties · mai</p>
          <p className="mt-2 text-body lg:text-h2 tabular-nums text-fg font-semibold">{formatEUR(totalOutflow)}</p>
        </Section>
      </div>
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-2">
        <div className="flex items-start justify-between gap-3">
          ...
          <p className={cn(
            ...
            {tx.type === 'inflow' ? '+' : ''}{formatEUR(tx.amountEur)}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className={cn('text-caption tabular-nums', lowConfidence ? 'text-warning' : 'text-fg-tertiary')}>
...
function PortfolioScreen() {
  ...
  return (
    <div className="flex flex-col gap-6 lg:gap-4">
        ...
        <Section ariaLabel="Valeur totale" className="lg:col-span-7">
          ...
          <p className="mt-2 flex items-center gap-1.5 text-body-sm tabular-nums">
            ...
            <span className="text-fg-tertiary">
              ({totalPnl >= 0 ? '+' : ''}{(totalPnlPct * 100).toFixed(2)} %) plus-value latente
            </span>
          </p>
        </Section>
      ...
      <Section ariaLabel="Lignes" title="Lignes" action={<HeaderAction icon={Plus} label="Ajouter" />}>
        <ul className="flex flex-col">
          {HOLDINGS.map((h) => (
            <li key={h.id}>
...
function HoldingRow({ holding }: { holding: typeof HOLDINGS[number] }) {
  ...
  return (
    <div className="flex items-center gap-3 py-3">
      ...
      <div className="text-right shrink-0">
        <p className="text-body-sm tabular-nums text-fg font-medium">{formatEUR(holding.marketValueEur)}</p>
    ...
    <div className="flex flex-col gap-6 lg:gap-4">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        <Section ariaLabel="Mois en cours" className="lg:col-span-7">
          <p className="text-caption text-fg-tertiary">{formatMonthYear(`${current.yearMonth}-15`)} · en cours</p>
          <div className="mt-3 grid grid-cols-3 gap-4">
            <Stat label="Entrées"    value={formatEUR(current.incomeEur)} />
            <Stat label="Sorties"    value={formatEUR(current.spendingEur)} />
            <Stat label="Net" value={signed(current.netEur)} tone={current.netEur >= 0 ? 'gain' : 'loss'} />
          </div>
        </Section>
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: 'gain' | 'loss' }) {
  return (
    <div>
      <p className="text-caption text-fg-tertiary">{label}</p>
      <p className={cn('mt-1 text-h3 tabular-nums', tone === 'gain' ? 'text-gain' : tone === 'loss' ? 'text-loss' : 'text-fg')}>
        {value}
      </p>
    </div>
  )
...
function MonthlyRow({ month }: { month: typeof MONTHLY[number] }) {
  return (
    <div className="flex items-center gap-3 py-3">
      <div className="flex-1 min-w-0">
        <p className="text-body-sm text-fg capitalize">{formatMonthYear(`${month.yearMonth}-15`)}</p>
        <p className="text-caption text-fg-tertiary tabular-nums">
          {formatEUR(month.incomeEur)} entrées · {formatEUR(month.spendingEur)} sorties
        </p>
      </div>
      <p className={cn('text-body-sm tabular-nums shrink-0 font-medium', month.netEur >= 0 ? 'text-gain' : 'text-loss')}>
        {signed(month.netEur)}
      </p>
      {month.signedOff && (
    ...
    <div className="flex flex-col gap-6 lg:gap-4">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <Section ariaLabel="Action" className="lg:col-span-5">
          ...
          <p className="mt-2 text-body lg:text-h2 tabular-nums text-fg font-semibold">{PROPERTIES.length} bien</p>
...
function PropertyCard({ property }: { property: typeof PROPERTIES[number] }) {
  ...
  return (
    <Section ariaLabel={property.label} title={property.label} action={
        ...
        {m && (
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-3">
              ...
              <Stat label="Reste"           value={`${Math.floor(m.termRemainingMonths / 12)} ans ${m.termRemainingMonths % 12} m`} />
            </div>
          </div>
        )}
      </div>
      ...
      <div className="mt-5 pt-5 border-t border-border">
        <div className="flex items-center gap-3">
          ...
          <p className="text-body-sm text-fg-tertiary tabular-nums">
            <span className="text-fg font-medium">{(equityRatio * 100).toFixed(0)} %</span> remboursé du capital initial
          </p>
        </div>
      </div>
    </Section>
    ...
    <div className="flex flex-col gap-6 lg:gap-4 lg:max-w-3xl">
      ...
      <Section ariaLabel="Intelligence artificielle" title="Intelligence artificielle">
        <p className="text-body-sm text-fg-secondary mb-4">
          Pekulo catégorise vos transactions automatiquement. Les modèles tournent par défaut sur votre appareil
          (Apple Intelligence) ou notre serveur privé (Ollama). Aucune donnée ne quitte ce périmètre sauf si vous
          activez explicitement les modèles tiers ci-dessous.
        </p>
        <ToggleRow
      ...
      <Section ariaLabel="Vos données" title="Vos données">
        <ul className="flex flex-col">
          <SettingRow
            ...
            action={<><Download size={14} strokeWidth={2} aria-hidden /> Exporter</>}
          />
          <SettingRow
            label="Supprimer mon compte"
            sub="Cascade sur toutes les tables · irréversible"
            destructive
            action={<><Trash2 size={14} strokeWidth={2} aria-hidden /> Supprimer</>}
          />
        </ul>
      </Section>
      ...
      <Section ariaLabel="Hypothèse" title="Hypothèse de projection">
        <ul className="flex flex-col">
          <SettingRow label="Versement mensuel" value={formatEUR(HYPOTHESIS.monthlyContributionEur)} />
          <SettingRow label="Rendement annuel supposé" value={`${HYPOTHESIS.assumedAnnualRatePct.toFixed(2)} %`} />
        </ul>
      </Section>
    </div>
  )
...
function SettingRow({
  label, value, sub, action, destructive,
}: {
  label: string; value?: string; sub?: string
  action?: React.ReactNode; destructive?: boolean
}) {
  return (
    <li className="flex items-center gap-4 py-3">
      <div className="flex-1 min-w-0">
        ...
        {value && !sub && <p className="text-caption text-fg-tertiary tabular-nums truncate">{value}</p>}
      </div>
      {action && (
        <button className={cn(
