...
- **AC-3 (PekuloError + error-mapper close the `app.ts:16` deferred-work marker):** **Given** `apps/api/src/common/errors/pekulo-error.ts` exports `PekuloError` (base class) and `apps/api/src/platform/http/error-mapper.ts` exports `mapErrorToOrpcResponse(err: unknown, requestId: string): { status: number; body: { error: { code: string; message: string; requestId?: string } } }`, **When** I run `bun --cwd apps/api test src/platform/http/error-mapper.test.ts`, **Then** all four cases pass with exit 0: (i) `new PekuloError("UNAUTHORIZED", "no session")` → `{ status: 401, body: { error: { code: "UNAUTHORIZED", message: "no session", requestId: <caller-id> } } }`, (ii) `new PekuloError("NOT_FOUND", "compass not found")` → `{ status: 404, ... }`, (iii) raw `new Error("boom")` → `{ status: 500, body: { error: { code: "INTERNAL", message: "internal server error", requestId: <caller-id> } } }` (message sanitised, requestId echoed from caller), (iv) thrown non-Error `"oops"` → `{ status: 500, body: { error: { code: "INTERNAL", ... } } }`. The caller (`apps/api/src/app.ts` `.onError`) generates the requestId via `crypto.randomUUID()` (RFC 4122 v4) BEFORE logging so the structured log line and the wire body share the same handle. Once `apps/api/src/common/ids/request-id.ts` ships in a future story, the caller swaps the helper without touching the mapper.
...
- [x] **Task 11 — Smoke run + curl `/rpc/v1/*` (AC-4 manual verification)** [AC: AC-4]
  ...
  ```
  Expected output (per module): `status=404`, body parses as JSON containing `"error":{"code":"NOT_FOUND","message":"no oRPC procedure matched /rpc/v1/<module>/noop","requestId":"<uuid-string>"}` (v4 today, v7 once the project helper lands). The `requestId` field varies per request — its presence is what's checked, not its value.
