...
**MODIFIED — post-flip hardening (commits `b32c51c` → `8e7327c`)**

_Surfaced by the aped-review git-audit pass — captured here so future story-spec checks against `git diff` line up. None of these were planned at story-write time ; they emerged from live-testing + the React-19 hydration / SA tag-boundary discoveries documented in the Post-implementation hardening section below._

- `apps/web/src/app/(cap)/dashboard/_components/cap-shell.tsx` (route nav + dialog lift — `b32c51c` + `902f4d3`)
- `apps/web/src/app/(cap)/dashboard/_components/compass-section.tsx` (hydration guard early-return — `77564c0`)
- `apps/web/src/app/(cap)/dashboard/_components/milestones-section.tsx` (hydration guard with `isInitialLoading` — `77564c0`)
- `apps/web/src/app/(cap)/dashboard/parametres/_components/accounts-section.tsx` (hydration guard — `77564c0`)
- `apps/web/src/app/(cap)/dashboard/parametres/_components/compass-history-panel.tsx` (hydration guard — `77564c0`)
- `apps/web/src/app/(cap)/dashboard/immobilier/_components/realestate-section.tsx` (hydration guard — `77564c0`)
- `apps/web/src/app/(cap)/dashboard/portefeuille/_components/portfolio-section.tsx` (hydration guard — `77564c0`)
- `apps/web/src/app/(cap)/dashboard/transactions/_components/transactions-stats-row.tsx` (NEW — ux-preview parity port `3955baa` ; hydration guard added `8e7327c`)
- `apps/web/src/app/(cap)/dashboard/transactions/_components/transactions-suggestions-section.tsx` (NEW — ux-preview parity port `3955baa`)
- `apps/web/src/app/(cap)/dashboard/_hooks/use-add-milestone-form.ts` + `use-delete-milestone.ts` + `use-update-milestone.ts` (`invalidateWithTags` back-port — `902f4d3`)
- `apps/web/src/app/(cap)/dashboard/immobilier/_hooks/use-attach-mortgage.ts` + `use-attach-rental.ts` + `use-create-property.ts` + `use-delete-property.ts` + `use-detach-mortgage.ts` + `use-detach-rental.ts` + `use-record-valuation.ts` + `use-update-mortgage.ts` + `use-update-rental.ts` (`invalidateWithTags` back-port — `902f4d3`)
- `apps/web/src/app/(cap)/dashboard/parametres/_hooks/use-create-account.ts` + `use-delete-account.ts` + `use-record-balance-change.ts` + `use-update-account.ts` + `use-update-compass.ts` (`invalidateWithTags` back-port — `902f4d3`)
- `apps/web/src/app/(cap)/dashboard/portefeuille/_hooks/use-close-holding.ts` + `use-create-holding.ts` + `use-record-lot.ts` (`invalidateWithTags` back-port — `902f4d3`)
- `apps/web/src/lib/orpc/modules.ts` (mount `transactionsClient` — T15 side-effect not in original File List)
- `apps/api/src/modules/accounts/accounts.integration.test.ts` (extend with `accountExists` integration probe — T9 side-effect)
- `apps/api/src/modules/accounts/accounts.service.test.ts` (3 new `accountExists` unit tests added during aped-review T9 close)
