...
- **L8 — Prisma 7 evaluates `env()` at every config-load (install + every prisma command).** `prisma.config.ts` resolves `env<Env>("DATABASE_URL")` at module-evaluation time, which fires on every `bun install` (apps/api postinstall = `prisma generate`) AND on `prisma format --check` / `validate` / `generate`. `pr.yml` sets `DATABASE_URL: "postgresql://stub:stub@localhost:5432/stub"` at **workflow level** so all jobs inherit it ; `rls-audit` overrides at step level with the real `RLS_AUDIT_DATABASE_URL` when configured. `apps/api/package.json#postinstall = "prisma generate"` ensures `apps/api/generated/prisma/` is materialised before TS compilation (typecheck job otherwise fails with `Cannot find module '@generated/prisma/client'`). Any future Dockerfile / pre-commit hook running `bun install` must apply the same stub at the broadest scope.
